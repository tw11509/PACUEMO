...
+ function($) {"use strict";
	...
	SidePanel.TOGGLE_BREAKPOINT = 992;
	SidePanel.MOBILE_BREAKPOINT = 480;
	SidePanel.DESKTOP_WIDTH = 400;
	SidePanel.MOBILE_WIDTH = 270;
	SidePanel.SIDEPANEL_PADDING = 38;
	SidePanel.CLOSE_DURATION = 270;
	SidePanel.MENU_ITEM_ANIMATION_DELAY = 17;
	SidePanel.DEFAULTS = {};
	SidePanel.prototype.setDimensions = function() {
		...
		if (_isTouchDevice()) {
			...
			if (window.innerWidth >= SidePanel.MOBILE_BREAKPOINT) {
				this.$element.css({
					width : toggleGutter + SidePanel.DESKTOP_WIDTH + "px"
				})
			} else {
				this.$element.css({
					width : toggleGutter + SidePanel.MOBILE_WIDTH + "px"
				})
			}
		}
		if (this.$toggleClone) {
	...
	SidePanel.prototype.show = function() {
		...
		if (this.animationEndEvent) {
			this.$element.find("li a, .divider").each(function(i, e) {
				$(e).css({
					"animation-delay" : i * SidePanel.MENU_ITEM_ANIMATION_DELAY + "ms"
				})
			})
		}
	...
	function Plugin(option) {
		return this.each(function() {
			...
			if (!data)
				$this.data("sp.sidepanel", data = new SidePanel(this, options));
